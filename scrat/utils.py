...
def get_item(url):
    ...
    element = driver.find_elements(
        By.CSS_SELECTOR, ".search-form.header-search-form > form > input:nth-child(2)"
    )
    ...
    title = driver.find_element(By.CSS_SELECTOR, "#br-pr-1 > h1").text

    colour = driver.find_element(
        By.CSS_SELECTOR, "div.series-item.series-color.current > a > ul > li > div"
    ).get_attribute("style")

    memory = driver.find_element(
        By.CSS_SELECTOR, ".br-pr-chr-item > div > div > span:nth-of-type(2) > a"
    ).text
    ...
    elements = driver.find_elements(
        By.CSS_SELECTOR, 'a[href="#reviews-list"].scroll-to-element > span'
    )
    reviews_count = element[0].text if elements else 0

    screen_size = driver.find_element(
        By.CSS_SELECTOR,
        "div.br-pr-chr > .br-pr-chr-item:nth-child(2) > div > div > span > a",
    ).text
    screen_power = driver.find_element(
        By.CSS_SELECTOR,
        "div.br-pr-chr > .br-pr-chr-item:nth-child(3) > div > div > span > a",
    ).text

    characteristic = {}
    for element in driver.find_elements(By.CSS_SELECTOR, ".br-pr-chr-item > div > div"):
        characteristic[
            element.find_element(By.CSS_SELECTOR, "span:first-of-type").text
